...
func (m *tmpModule) Setup() {
	m.out = make(chan AnalyzableStory, 1)
	m.closing = make(chan chan error)
}

func (m *tmpModule) Close() error {
	errc := make(chan error)
	m.closing <- errc
	return <-errc
}

func (m *tmpModule) SetInputChan(inc chan AnalyzableStory) {
	m.in = inc
}

func (m *tmpModule) GetOutputChan() chan AnalyzableStory {
	return m.out
}

func (m *tmpModule) SetErrorPropogateChan(errc chan error) {
	m.err = errc
}

// check that the module was compiled properly
var _ Module = (*tmpModule)(nil)
